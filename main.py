...
def predict_route():
    data_temp = request.json
   
    # Access the symptoms data using the appropriate key
    symptoms_data = data_temp.get('symptom')
    
    # Ensure symptoms_data is not None
    if symptoms_data:
        user_symptoms = [s.strip() for s in symptoms_data.split(',')]
        ...
        pre = [list(p) for p in pre]  # Convert each row of precautions to a list
        med = list(med)               # Convert medications to a list
        die = list(die)               # Convert diet to a list
        wrkout = list(wrkout)         # Convert workout to a list
...
# Function to start the Flask app and expose it using ngrok
def start_ngrok():
